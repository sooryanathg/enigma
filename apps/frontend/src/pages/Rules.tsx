...
const RULES: Rule[] = [
  ...
  { number: 4, title: "Timer", description: "The second you create your account, the game begins. The clock’s been your enemy all along — you just didn’t know it." },
  ...
  if (isMobile) {
    return (
      <div className="relative w-full bg-black border border-white px-4 py-6 sm:px-6 sm:py-8">
        <div className="absolute top-4 sm:top-6 left-4 sm:left-6 font-whirlyBirdie font-bold text-white text-3xl sm:text-4xl md:text-5xl leading-tight">{rule.number}</div>
        <div className="absolute top-4 sm:top-6 left-16 sm:left-20 md:left-24 font-whirlyBirdie font-bold text-white text-lg sm:text-xl md:text-2xl leading-tight">{rule.title}</div>
      ...
      <div className="lg:hidden px-4 sm:px-6 md:px-8 py-8 sm:py-10 md:py-12">
        <div className="max-w-4xl mx-auto space-y-4 sm:space-y-6">
          <div className="bg-black px-4 sm:px-6 py-3 sm:py-4 mb-6 sm:mb-8">
            <h1 className="text-white font-whirlyBirdie font-bold text-xl sm:text-2xl md:text-3xl">RULES</h1>
          </div>
          <h2 className="font-whirlyBirdie font-bold text-center text-black text-2xl sm:text-3xl md:text-4xl lg:text-5xl tracking-tight mb-2 sm:mb-3">game rules</h2>
